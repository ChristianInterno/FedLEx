...
from src.plot_utils import get_class_distributions, plot_class_distributions
...
def simulate_split(args, dataset):
    ...
    # Non-IID split proposed in (McMahan et al., 2016); each client has samples from at least two different classes
        ...
        distributions = get_class_distributions(split_map, dataset)
        plot_class_distributions(distributions, args.num_classes,
                                 save_path_distri=os.path.join(args.result_path, 'Distribution_Clients'))
        return split_map
    ...
    # Non-IID split proposed in (Hsu et al., 2019); simulation of non-IID split scenario using Dirichlet distribution
        ...
        # Plot the class distributions after splitting
        distributions = get_class_distributions(split_map, dataset)
        plot_class_distributions(distributions, args.num_classes, save_path_distri = os.path.join(args.result_path, 'Distribution_Clients'))
